...
- (void)monsterAttackAction {
    ...
    [self runAction:attackAction completion:^{
     ...
     [self performSelector:@selector(createFireAttackAction) withObject:nil afterDelay:0.1 * 3];
    
}

- (void)createFireAttackAction{
    
    NSLog(@"%lf",self.xScale);
    CGFloat x = 100;
    if (self.xScale < 0) {
        NSLog(@"向左");
        x = -100;
    ...
    fireNode.position = CGPointMake(self.position.x + x, self.position.y);
